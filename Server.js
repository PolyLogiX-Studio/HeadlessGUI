...
const DefaultConfig = {
	"comment":"This config was generated by HeadlessCORE",
	"tickRate": 60.0,
...
const {store,config} = require(path.join(__dirname,"Pages/Resources/store"))
...
class Server {
	...
	constructor(session){
			...
			fs.mkdirSync(session.sessionsDir)
		}
		this.Config = DefaultConfig
		this.ID = session.UUID
        this.sessionName = session.sessionName
        this.accessLevel = session.accessLevel.replace(' ','')
...
class Instances {
	...
	newSession(session) {
		...
		this.Instances[session.UUID] = new Server(session)
