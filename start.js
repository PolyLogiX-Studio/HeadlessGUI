...
app.on('ready', function() {
    ...
    mainWindow = new BrowserWindow({
        width: 1920,
        height: 1080,
        icon: path.join(__dirname, 'HeadlessGraphic.png'),
        webPreferences: {
...
function createAddWindow() {
    ...
    addWindow = new BrowserWindow({
        ...
        height: 800,
        title: "New Server",
        icon: path.join(__dirname, 'GraphicIcon_-_Golden_Neos.png'),
        webPreferences: {
            nodeIntegration: true
        }
    });
...
function createAddWindowAdvanced() {
    ...
    addWindowAdvanced.loadURL(url.format({
        pathname: path.join(__dirname, 'addWindowAdvanced.html'),
        protocol: 'file:',
        slashes: true
    }))
...
function createURLWindow(URL, width = 1080, height = 1080) {
    let URLwindow = new BrowserWindow({
        ...
        height: height,
        title: "Config",
        icon: path.join(__dirname, 'polylogix.jpg'),
        webPreferences: {
...
class Server {
    ...
    openWindow() {
        ...
        this.Console.loadURL(url.format({
            pathname: path.join(__dirname, `ServerManager.html`),
            protocol: 'file:',
            slashes: true,
        })+`?id=${this.ID}`);
