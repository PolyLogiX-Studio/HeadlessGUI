...
checkInternet(function (isConnected) {
  if (isConnected) {
    ...
    if (
      store.has('NEOS:token') &&
      new Date(store.get('NEOS:token:expire')) > new Date()
    ) {
...
function sendLoginPost(loginPayload) {
  ...
  return fetch(CLOUDX_PRODUCTION_NEOS_API + 'api/userSessions', {
    ...
    .then((json) => {
      ...
      store.set(
        'loginCredentials',
        loginPayload.email ? loginPayload.email : loginPayload.username
      );
      store.set(
        'loginPassword',
        loginPayload.password
          ? loginPayload.password
          : store.get('loginPassword')
      );
    ...
    .catch((err) => {
      ...
      if (!store.get('offlineMode')) {
        //Dont clear Credentials if offline
