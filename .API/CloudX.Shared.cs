      ...
      30f
        ...
        public UserSession CurrentSession
        {
            ...
            private set
            {
                ...
                lock (this.lockobj)
                {
                    this._lastSessionUpdate = DateTime.Now;
        ...
        public virtual void Update()
        {
            lock (this.lockobj)
            {
                ...
                {
                    if ((DateTime.Now - this._lastSessionUpdate).TotalSeconds >= 3600.0)
                    {
                        Task.Run<CloudResult>((Func<Task<CloudResult>>)(async () => await this.ExtendSession()));
                        this._lastSessionUpdate = DateTime.Now;
                    }
                }
            }
        ...
        public async Task<CloudResult<T>> GET<T>(string resource, TimeSpan? timeout = null) where T : class, new()
        {
            return await this.RunRequest<T>(this.CreateRequest(resource, HttpMethod.Get), timeout).ConfigureAwait(false);
        }

        public async Task<CloudResult<T>> POST<T>(
          string resource,
          object entity,
          ...
          where T : class, new()
        {
            ...
            return await this.RunRequest<T>(request, timeout).ConfigureAwait(false);
        }

        public async Task<CloudResult<T>> POST_File<T>(
          string resource,
          string filePath,
        ...
        {
            HttpRequestMessage request = this.CreateRequest(resource, HttpMethod.Post);
            CloudResult<T> cloudResult;
            using (this.AddFileToRequest(request, filePath, fileMIME, progressIndicator))
                cloudResult = await this.RunRequest<T>(request, new TimeSpan?(TimeSpan.FromMinutes(60.0))).ConfigureAwait(false);
            return cloudResult;
        }

        public async Task<CloudResult<T>> PUT<T>(
          string resource,
          object entity,
          TimeSpan? timeout = null)
          where T : class, new()
        ...
        public async Task<CloudResult<T>> PATCH<T>(
          string resource,
          object entity,
          TimeSpan? timeout = null)
          where T : class, new()
        {
            HttpRequestMessage request = this.CreateRequest(resource, new HttpMethod(nameof(PATCH)));
            this.AddBody(request, entity);
            return await this.RunRequest<T>(request, timeout).ConfigureAwait(false);
        }

        public async Task<CloudResult<T>> DELETE<T>(string resource, TimeSpan? timeout = null) where T : class, new()
        {
            return await this.RunRequest<T>(this.CreateRequest(resource, HttpMethod.Delete), timeout).ConfigureAwait(false);
        }

        public async Task<CloudResult> GET(string resource, TimeSpan? timeout = null)
        {
            return await this.RunRequest(this.CreateRequest(resource, HttpMethod.Get), timeout).ConfigureAwait(false);
        }

        public async Task<CloudResult> POST(
          string resource,
          object entity,
          TimeSpan? timeout = null)
        {
            HttpRequestMessage request = this.CreateRequest(resource, HttpMethod.Post);
            this.AddBody(request, entity);
            return await this.RunRequest(request, timeout).ConfigureAwait(false);
        }

        public async Task<CloudResult> POST_File(
          string resource,
          string filePath,
          string fileMIME = null,
          IProgressIndicator progressIndicator = null)
        {
            ...
            using (this.AddFileToRequest(request, filePath, fileMIME, progressIndicator))
                cloudResult = await this.RunRequest(request, new TimeSpan?(TimeSpan.FromMinutes(60.0))).ConfigureAwait(false);
            return cloudResult;
        }

        public async Task<CloudResult> PUT(
          string resource,
          object entity,
          TimeSpan? timeout = null)
        {
        ...
        public async Task<CloudResult> PATCH(
          string resource,
          object entity,
          TimeSpan? timeout = null)
        {
        ...
        public async Task<CloudResult> DELETE(string resource, TimeSpan? timeout = null)
        {
            return await this.RunRequest(this.CreateRequest(resource, HttpMethod.Delete), timeout).ConfigureAwait(false);
        }

        private FileStream AddFileToRequest(
          ...
          where T : class
        {
            ...
            HttpResponseMessage result = await this.HttpClient.SendAsync(request, cancellationTokenSource.Token).ConfigureAwait(false);
            T entity = default(T);
        ...
        private async Task<CloudResult> RunRequest(
          ...
          TimeSpan? timeout)
        {
            return (CloudResult)await this.RunRequest<string>(request, timeout).ConfigureAwait(false);
        }
          ...
          long? bytes = null)
        {
            ...
            using (FileStream fileStream = File.OpenRead(assetPath))
            {
                ...
                {
                    bool flag;
                    if (enqueuedChunks < assetUpload.TotalChunks)
                    {
                        CloudXInterface.UploadChunkBuffer last = freeBuffers.TakeLast<CloudXInterface.UploadChunkBuffer>();
                        int count = fileStream.Read(last.data, 0, last.data.Length);
                        HttpRequestMessage request = this.CreateRequest(baseUrl + "/" + enqueuedChunks.ToString(), HttpMethod.Post);
                        MultipartFormDataContent multipartFormDataContent = new MultipartFormDataContent();
                        ByteArrayContent byteArrayContent = new ByteArrayContent(last.data, 0, count);
                        byteArrayContent.Headers.ContentLength = new long?((long)count);
                        multipartFormDataContent.Add((HttpContent)byteArrayContent, "file", fileName);
                        request.Content = (HttpContent)multipartFormDataContent;
                        if (CloudXInterface.DEBUG_UPLOAD)
                            UniLog.Log(string.Format("Enqueuing chunk {0}.", (object)enqueuedChunks), false);
                        last.task = this.RunRequest(request, new TimeSpan?());
                        processingBuffers.Add(last);
                        ++enqueuedChunks;
                    ...
                    if (flag)
                    {
                        ...
                        if (CloudXInterface.DEBUG_UPLOAD)
                            UniLog.Log("Chunk result: " + cloudResult?.ToString(), false);
